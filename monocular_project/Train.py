...
model = net.BtsController(experiment_name, backprop_frequency=effective_batch_size / batch_size)
...
start_epoch = 0
if os.path.exists(os.path.join(saved_models_path, "model_latest")):
    model.load_model(os.path.join(saved_models_path, "model_latest"))
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        if i_batch > skip_step:
            ...
            if i_batch % 1000 == 999:
                ...
                model.save_model(os.path.join(saved_models_path, "model_latest"))
